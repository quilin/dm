...
{
    /// <inheritdoc />
    public class CreateAttributeSchemaValidator : AbstractValidator<CreateAttributeSchema>
    {
        /// <inheritdoc />
                        ...
                        .Custom((constraints, ctx) =>
                        {
                            switch (constraints)
                            {
                                ...
                                case StringAttributeConstraints stringConstraints:
                                    ...
                                    {
                                        ctx.AddFailure(nameof(StringAttributeConstraints.MaxLength), ValidationError.Invalid);
                                    }
                                    break;
                                case ListAttributeConstraints listConstraints:
                                    if (listConstraints.Values.Length == 0)
                                    {
                                        ctx.AddFailure(nameof(ListAttributeConstraints.Values), ValidationError.Invalid);
                                    }
