...
public class PollController(
    IPollApiService apiService) : ControllerBase
{
    /// <summary>
    /// Get list of polls
    /// </summary>
    ...
    public async Task<IActionResult> GetPolls([FromQuery] PollsQuery q) =>
        Ok(await apiService.Get(q, HttpContext.RequestAborted));
    ...
    {
        var result = await apiService.Create(poll, HttpContext.RequestAborted);
    ...
    public async Task<IActionResult> GetPoll(Guid id) =>
        Ok(await apiService.Get(id, HttpContext.RequestAborted));
    ...
    public async Task<IActionResult> PutPollVote(Guid id, [FromQuery] Guid optionId) =>
        Ok(await apiService.Vote(id, optionId, HttpContext.RequestAborted));
}