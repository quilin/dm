...
import { Comment, Forum, Topic } from '@/api/models/forum';
import Api from '@/api';
...
export default new class ForumApi {
  ...
  public async postComment(id: string, comment: Comment): Promise<ApiResult<Envelope<Comment>>> {
    return await Api.post(`topics/${id}/comments`, comment);
  }

  public async updateComment(id: string, comment: Comment): Promise<ApiResult<Envelope<Comment>>> {
  ...
  public async getCommentForUpdate(id: string): Promise<Comment> {
    const { data } = await Api.get<Envelope<Comment>>(`forum/comments/${id}`, undefined, BbRenderMode.Bb);
